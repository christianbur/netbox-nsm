...
- **IP Analysis standalone page** — the two-column compare UI at `/plugins/netbox-nsm/ip-analysis/` (`IPAnalysisView`, `ip_analysis.html`, `ipa_object_analysis_table.html`) is removed. IP address resolution now lives only in the **IP Analyzer applet** on rule detail pages and via the analysis APIs. The old URL permanently redirects to **Object Analyzer**; legacy `ip_ct` / `ip_pk` / `ip_name` query params pre-select the first object when present. Unused `nsm_ip_analysis_url` context from the Security tab is dropped.
...
- **IP Analyzer applet — stuck loading spinner** — closing the popup or invalidating an in-flight analysis no longer leaves the active tab on “Analysis running…” forever; failed or superseded loads surface `.nsm-ipa-applet-error` with a readable message (timeout, HTTP status, or server error text).
- **IP Analyzer — collapsed root groups template crash** — `_attach_ipa_cell_group_collapse_hints` no longer strips `collapsed_group_count` from the synthetic `collapsed_root_groups` wrapper (cells with many address groups triggered `blocktrans` `TemplateSyntaxError` and a 500 “Analysis failed.”). The UI API now includes the exception short text in the JSON `error`/`detail` fields when rendering fails.
