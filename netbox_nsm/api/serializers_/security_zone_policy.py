...
class SecurityZonePolicySerializer(NetBoxModelSerializer):
    url = HyperlinkedIdentityField(
        view_name="plugins-api:netbox_nsm-api:securityzonepolicy-detail"
    )
    source_zone = SecurityZoneSerializer(
    ...
    def validate(self, data):
        ...
        if source_zone and destination_zone and set(source_zone) & set(destination_zone):
            message = (
    ...
    def create(self, validated_data):
        source_zone = validated_data.pop("source_zone", None)
        destination_zone = validated_data.pop("destination_zone", None)
        ...
        if source_zone is not None:
            policy.source_zone.set(source_zone)
        if destination_zone is not None:
            policy.destination_zone.set(destination_zone)
