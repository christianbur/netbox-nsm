"""Portable schema helpers — load bundled schema from ``bundles/builtin/`` via ``paths.py``."""
...
def type_slug(base_name):
    """``"Address Group"`` -> ``"nsm_address_group"``."""
    ident = slugify_identifier(base_name)
    if ident.startswith("nsm_"):
        return ident
    return f"nsm_{ident}"
...
def _schema_bundle_path() -> Path:
    from netbox_nsm.bundles.paths import bundle_json_path

    return bundle_json_path("nsm_schema")


def _read_schema_bundle() -> dict:
    path = _schema_bundle_path()
    with path.open(encoding="utf-8") as fh:
...
def load_portable_schema_document(
    *,
    ...
    payload = {
        ...
        "choice_sets": list(document.get("choice_sets") or []),
        "types": types,
        "objects": list(document.get("objects") or []),
    }
    if slugs is None:
        return payload
    payload["types"] = [t for t in types if t.get("slug") in slugs]
    return payload


def load_choice_set_specs() -> list[dict]:
    """Load bundled choice-set definitions for schema apply."""
    data = _read_schema_bundle().get("choice_sets") or []
    if not isinstance(data, list):
        raise ValueError(f"{_schema_bundle_path().name}: choice_sets must be a JSON array")
    return data
