...
def detect_instance_db_alias(instance) -> str | None:
    ...
    After ``PrimaryModelForm.save()`` a new branch-aware row may exist only in a branch
...
def required_junction_db_alias(
    ...
    probe_model = get_object_link_model()
    if probe_model is not None:
        routed = router_write_alias(probe_model)
        if routed:
            return routed
...
def junction_transaction(
    ...
    """Run junction-table writes on the same DB alias as the parent instance."""
