...
_attach(f_act, tc_act)

# Build zone name list in order
zone_names_ordered = [z[0] for z in ZONE_DEFS if z[0] != "infrastructure"]  # 10 zones
...
for i, z1 in enumerate(zone_names_ordered):
    for z2 in zone_names_ordered[i+1:]:
        ...
        svc = svcs_by_name["HTTPS"]

        for src, dst, direction in [(z1, z2, "fwd"), (z2, z1, "rev")]:
            rname = f"{src}-to-{dst}"
            r = _rule(rb_ts, rname, idx)
            _item(r, f_src, zones_by_name[src], zone_ct)
            _item(r, f_dst, zones_by_name[dst], zone_ct)
            _item(r, f_svc, svc, svc_ct)
            _item(r, f_act, actions_by_name[action_key], act_ct)
            idx += 10
            rule_count += 1
...
_attach(fi_act, tc_act)

infra_zone = zones_by_name["infrastructure"]
idx = 10
for zname in zone_names_ordered:
    rname = f"infra-to-{zname}"
    r = _rule(rb_ti, rname, idx)
    _item(r, fi_src, infra_zone, zone_ct)
    _item(r, fi_dst, zones_by_name[zname], zone_ct)
    _item(r, fi_svc, svcs_by_name["LDAP"], svc_ct)
    _item(r, fi_act, actions_by_name["permit"], act_ct)
    idx += 10

# Infra ↔ Infra
r = _rule(rb_ti, "infra-to-infra", idx)
_item(r, fi_src, infra_zone, zone_ct)
_item(r, fi_dst, infra_zone, zone_ct)
_item(r, fi_svc, svcs_by_name["LDAP"], svc_ct)
_item(r, fi_act, actions_by_name["permit"], act_ct)
print(f"     ✓ 11 rules")
...
def _irule(name, idx, src_lbl, dst_lbl, svc_name, action_key):
    ...
    if src_lbl and src_lbl in labels_by_key:
        _item(r, il_src, labels_by_key[src_lbl], DjCT.objects.get_for_model(LabelModel))
    if dst_lbl and dst_lbl in labels_by_key:
        _item(r, il_dst, labels_by_key[dst_lbl], DjCT.objects.get_for_model(LabelModel))
...
DC_RULES = [
    ...
    *[(f"prod-to-{z}-api", "prod", z, "App-HTTPS", "permit")
    ...
    ("all-zones-to-dns-fwd", None, "infrastructure", "DNS-UDP", "permit"),
    # NTP: all zones → infra NTP
    ("all-zones-to-ntp", None, "infrastructure", "NTP", "permit"),
]
...
for i, rule_def in enumerate(DC_RULES):
    ...
    if dst_zname and dst_zname in zones_by_name:
        _item(r, dc_dst, zones_by_name[dst_zname], zone_ct)
    _item(r, dc_svc, svcs_by_name[svc_name], svc_ct)
    _item(r, dc_act, actions_by_name[action_key], act_ct)
print(f"     ✓ {len(DC_RULES)} rules")
...
for i, (rname, src_n, dst_n, svc_n, act_n) in enumerate(MGMT_RULES):
    ...
    if src_n in zones_by_name:
        ...
        _item(r, mg_src, addrs_by_name[src_n], addr_ct)
    if dst_n in addrs_by_name:
        _item(r, mg_dst, addrs_by_name[dst_n], addr_ct)
    _item(r, mg_svc, svcs_by_name[svc_n], svc_ct)
    _item(r, mg_act, actions_by_name[act_n], act_ct)
print(f"     ✓ {len(MGMT_RULES)} rules")
...
# ── 8g. SASE, Internet Outer/Inner, GCP DMZ, VPN ─────────────────────────────
...
for rb_name, rules in SIMPLE_RBS:
    ...
    for i, (rname, src_n, dst_n, svc_n, act_n) in enumerate(rules):
        ...
        for field, name, ct in [(x_src, src_n, None), (x_dst, dst_n, None)]:
            if name is None:
                continue
            obj = addrs_by_name.get(name) or zones_by_name.get(name)
            fct = addr_ct if name in addrs_by_name else zone_ct
            if obj:
                _item(r, field, obj, fct)
        _item(r, x_svc, svcs_by_name[svc_n], svc_ct)
