...
class SecurityObjectForm(PrimaryModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            try:
                ct = SecurityObjectType.objects.get(pk=self.data["custom_type"])
            except (SecurityObjectType.DoesNotExist, ValueError):
                pass
        elif "custom_type" in (self.initial or {}):
            try:
                ct = SecurityObjectType.objects.get(pk=self.initial["custom_type"])
            except (SecurityObjectType.DoesNotExist, ValueError):
                pass
        if ct:
...
class SecurityObjectFilterForm(PrimaryModelFilterSetForm):
    model = SecurityObject
    fieldsets = (
...
class SecurityObjectBulkEditForm(PrimaryModelBulkEditForm):
    model = SecurityObject
    description = forms.CharField(max_length=200, required=False)
    tags = TagFilterField(model)
