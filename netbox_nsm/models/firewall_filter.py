...
from netbox_nsm.constants import FILTER_ASSIGNMENT_MODELS
...
class FirewallFilter(ContactsMixin, PrimaryModel):
    ...
    def get_absolute_url(self):
        return reverse("plugins:netbox_nsm:firewallfilter", args=[self.pk])
...
class FirewallFilterAssignment(NetBoxModel):
    ...
    firewall_filter = models.ForeignKey(
        to="netbox_nsm.FirewallFilter", on_delete=models.CASCADE
    )

    clone_fields = ("assigned_object_type", "assigned_object_id")

    prerequisite_models = ("netbox_nsm.FirewallFilter",)
