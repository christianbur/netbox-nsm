...
class NsmObjectType(PrimaryModel):
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                Lower("name"),
                name="netbox_nsm_nsmobjecttype_name_ci_unique",
            )
        ]
    ...
    def get_absolute_url(self):
        return reverse("plugins:netbox_nsm:nsmobjecttype", args=[self.pk])


class NsmObjectTypeField(PrimaryModel):
    nsm_object_type = models.ForeignKey(
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=("nsm_object_type", "name"),
                name="netbox_nsm_nsmobjecttypefield_unique_name",
            )
        ]
    ...
    def get_absolute_url(self):
        return reverse("plugins:netbox_nsm:nsmobjecttypefield", args=[self.pk])


class NsmObject(PrimaryModel):
    nsm_object_type = models.ForeignKey(
        to="netbox_nsm.NsmObjectType",
        on_delete=models.CASCADE,
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=("nsm_object_type", "name"),
                name="netbox_nsm_nsmobject_unique_type_name",
            ),
            models.UniqueConstraint(
                fields=("nsm_object_type", "source_model", "source_pk"),
                name="netbox_nsm_nsmobject_unique_type_source",
            ),
        ]
    ...
    def get_absolute_url(self):
        return reverse("plugins:netbox_nsm:nsmobject", args=[self.pk])
    ...
    def clean(self):
        ...
        field_defs = list(self.nsm_object_type.fields.all())
        ...
        for field_def in field_defs:
            ...
            if field_def.unique and not self._is_empty_value(value):
                qs = self.__class__.objects.filter(
                    nsm_object_type=self.nsm_object_type,
    ...
    def get_typed_object_data(self):
        rows = []
        for field_def in self.nsm_object_type.fields.all():
            rows.append(
                {
                    "name": field_def.name,
...
@register_search
class NsmObjectTypeIndex(SearchIndex):
    model = NsmObjectType
    fields = (
        ("name", 200),
        ("slug", 200),
...
@register_search
class NsmObjectIndex(SearchIndex):
    model = NsmObject
    fields = (
        ("name", 300),
        ("description", 500),
