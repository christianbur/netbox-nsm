...
def find_object_link_between(object_a, object_b):
    """Return an ObjectLink between two objects in either direction, if any."""
    ...
    link = ObjectLink.objects.filter(
        object_a_type=ct_a,
        object_a_id=object_a.pk,
        object_b_type=ct_b,
        object_b_id=object_b.pk,
    ).first()
    if link is not None:
        return link
    return ObjectLink.objects.filter(
        object_a_type=ct_b,
        object_a_id=object_b.pk,
        object_b_type=ct_a,
        object_b_id=object_a.pk,
    ).first()
...
def object_link_assign_url(
    ...
    *,
    ...
    if link is not None:
        ...
        params["propagation"] = link.propagation
        if link.propagate_stop_on_own:
            params["propagate_stop_on_own"] = "1"
...
def address_ipam_fk_clear_url(
    ...
    from netbox_nsm.address_ipam_fk import NSM_ADDRESSES_SLUG
