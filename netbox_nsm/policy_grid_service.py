"""Server-side policy AG Grid data (Community infinite row model)."""
...
from netbox_nsm.policy_grid_filter import parse_filter_model_json
...
POLICY_GRID_BLOCK_SIZE = 100
POLICY_GRID_RULES_CACHE_TTL = 600
...
def _group_assign_cache_key(
    ...
    return f"nsm:policy_grid:group_map:{pks_sig}:{primary}:{secondary}"
...
def policy_grid_rules_cache_key(rulebook_pk: int, filter_model) -> str:
    return (
        f"nsm:policy_grid:rules:{rulebook_pk}:"
...
def policy_grid_filtered_rules(rulebook):
    return prepare_rules(_base_rules_qs(rulebook))


def policy_grid_column_defs(rulebook, view_helpers) -> list[dict]:
    grouped = view_helpers._build_grouped_policy_table_data([], rulebook)
    payload = build_policy_ag_grid_column_defs(grouped)
    return payload["columnDefs"]


def wrap_policy_ag_row_urls(records: list[dict], request) -> None:
...
def _rules_after_ag_filter(rules, rulebook, filter_model, view_helpers) -> list:
    grouped = view_helpers._build_grouped_policy_table_data(rules, rulebook)
    ...
    for rule in rules:
        ...
        if row is None:
            continue
        record = build_policy_ag_grid_row(row)
        if apply_ag_grid_row_filter([record], filter_model):
            filtered.append(rule)
    return filtered


def _build_grouped_policy_page(
    rules,
    rulebook,
    *,
    start_row: int,
    ...
    grouped = view_helpers._build_grouped_policy_table_data(page_rules, rulebook)
    rows_by_pk = {row["pk"]: row for row in grouped.get("rows") or []}
...
def resolve_policy_grid_filtered_rules(
    ...
    *,
    ...
    cache_key = policy_grid_rules_cache_key(rulebook.pk, filter_model)
    if refresh_cache:
    ...
    rules = policy_grid_filtered_rules(rulebook)
    if filter_model:
...
def resolve_all_rules_filtered_rules(
    ...
    *,
    ...
    from netbox_nsm.policy_grid_payload import apply_ag_grid_row_filter
...
def fetch_policy_grid_page(
    ...
    *,
    ...
    if group_levels:
        page, total = _build_grouped_policy_page(
            rules,
            rulebook,
            start_row=start_row,
            end_row=end_row,
        ...
        wrap_policy_ag_row_urls(page, request)
    elif filter_model:
        grouped = view_helpers._build_grouped_policy_table_data(rules, rulebook)
        records = [build_policy_ag_grid_row(row) for row in grouped.get("rows") or []]
        ...
        grouped = view_helpers._build_grouped_policy_table_data(chunk, rulebook)
        page = [build_policy_ag_grid_row(row) for row in grouped.get("rows") or []]
        wrap_policy_ag_row_urls(page, request)
