"""Rulebook detail panel: assigned hosts and their Security Panel interface links."""
...
from netbox_nsm.panel_link_actions import append_return_url, object_link_assign_url
from netbox_nsm.security_panel_links import build_object_link_rows

__all__ = ("build_rulebook_assigned_objects_panel",)
...
def build_rulebook_assigned_objects_panel(rulebook, request) -> dict:
    return_url = request.path if request else rulebook.get_absolute_url()
    if request:
    ...
    add_url = reverse("plugins:netbox_nsm:rulebook_bulk_assign", args=[rulebook.pk])
    if request:
        add_url = with_branch_query(add_url, request)

    hosts: list[dict] = []
    assignments = rulebook.assignments.select_related(
        "assigned_object_type", "rulebook"
    ...
    for assignment in assignments:
        ...
        if can_delete:
            remove_url = append_return_url(
                reverse(
                    "plugins:netbox_nsm:rulebookassignment_delete",
