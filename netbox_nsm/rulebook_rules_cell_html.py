...
def _wrap_rules_cell_list(items, inner_html: str, mode_class: str) -> str:
...
def _render_rules_object_cell_inline(items, *, colored=True) -> str:
    ...
    return _wrap_rules_cell_list(
        items, _join_inline_cell_items(parts), "nsm-ag-cell-list--inline"
    )


def _render_rules_object_cell_pill_more(items, *, colored=True) -> str:
    if len(items) <= 1:
        return _render_rules_object_cell_stack(items, colored=colored)
    shown = items[:1]
    hidden = items[1:]
    ...
    inner = "".join(parts)
    return _wrap_rules_cell_list(items, inner, "nsm-ag-cell-list--pill-more")


def _render_rules_object_cell_compact(items, *, colored=True) -> str:
    if len(items) < 2:
        return _render_rules_object_cell_stack(items, colored=colored)
    ...
    return _wrap_rules_cell_list(items, inner, "nsm-ag-cell-list--compact")
...
def render_rules_object_cell_html(
    ...
    *,
    ...
    if mode == CELL_MODE_INLINE:
        return _render_rules_object_cell_inline(items, colored=colored)
    if mode == CELL_MODE_COMPACT:
        return _render_rules_object_cell_compact(items, colored=colored)
    if mode == CELL_MODE_PILL_MORE:
        return _render_rules_object_cell_pill_more(items, colored=colored)
    return _render_rules_object_cell_stack(items, colored=colored)
