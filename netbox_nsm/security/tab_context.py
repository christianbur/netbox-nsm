...
def panel_link_payload(linked, lct, tmpl_map, **extra):
    ...
    payload.setdefault(
        "row_type_filter_key",
        payload.get("cot_slug") or payload.get("row_type_label") or "",
    )
    return payload
...
def build_security_tab_context(obj, request) -> dict:
                ...
                for _addr_obj, _fk_field_name in iter_addresses_for_ipam_object(obj):
                    ...
                    links_by_type[_addr_type_key]["objects"].append(
                        panel_link_payload(
                            ...
                            field_label=_fk_field_name,
                            row_type_label=_link_type_label(_addr_ct),
                            **address_ipam_fk_ref_action_urls(
                                obj,
                                _addr_obj,
                                _fk_field_name,
            ...
            for _ref in iter_address_ipam_fk_refs(obj):
                ...
                links_by_type[_ipam_type_key]["objects"].append(
                    panel_link_payload(
                        ...
                        source="ipam_fk",
                        source_label=src_ipam,
                        field_label=_ref.field_name,
                        row_type_label=_link_type_label(_ipam_ct),
                        **address_ipam_fk_action_urls(
                            obj,
                            _ref.field_name,
                            _ipam_obj,
        ...
        def _add_group_m2m_link(related, comment, **action_urls):
            ...
            links_by_type[_grp_type_key]["objects"].append(
                panel_link_payload(
                    ...
                    field_label=comment,
                    row_type_label=_link_type_label(ct),
                    **action_urls,
                )
            )
    ...
    badge_total = total_links
    if nsm_enforcement_point:
        badge_total += int(nsm_enforcement_point.get("count") or 0)
    badge_total += sum(int(i.get("entry_count") or 0) for i in nsm_interface_analysis)
    security_badge = badge_total or None
