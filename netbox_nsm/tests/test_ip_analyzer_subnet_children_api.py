...
class IpAnalyzerSubnetChildrenApiTests(SimpleTestCase):
    ...
    @patch(
        "netbox_nsm.analyzers.ip_analyzer.endpoints.subnet_children_api._finalize_ipa_cell_object_tree_lazy"
    )
    @patch(
        "netbox_nsm.analyzers.ip_analyzer.endpoints.subnet_children_api._build_ipa_object_tree_node"
    )
    @patch(
    ...
    def test_renders_lazy_subnet_children_with_depth_and_enrichment(
        ...
        raw_node = {"name": "10.0.1.0/24", "children": []}
        build_node_fn.return_value = raw_node
        ...
        finalize_fn.return_value = finalized_nodes
        ...
        finalize_args = finalize_fn.call_args[0]
        self.assertTrue(finalize_args[0][0].get("is_cell_direct"))
        self.assertTrue(finalize_args[0][0].get("in_cell"))
        self.assertEqual(finalize_args[1], {(42, 11)})
        self.assertIn((42, 11), finalize_args[2])
        self.assertIs(finalize_args[2][(42, 11)], child_prefix_obj)

        self.assertTrue(finalized_nodes[0].get("subnet_contained_dup"))
        self.assertEqual(finalized_nodes[0].get("subnet_contained_in"), "10.0.0.0/8")
        self.assertEqual(finalized_nodes[0].get("subnet_contained_in_name"), "ANY")
        self.assertEqual(finalized_nodes[0].get("subnet_contained_in_url"), "/ipam/prefixes/7/")

        mark_duplicate_fn.assert_called_once_with(finalized_nodes, is_root=True)
        attach_dup_status_fn.assert_called_once_with(finalized_nodes)
        attach_dup_context_fn.assert_called_once_with(finalized_nodes)
